...
A Receiver sets the `rtp_enabled` parameter to true for each leg it has configured from the SDP file.
...
Where a Receiver supports SMPTE 2022-7 but receives a request with a non-SMPTE 2022-7 transport file, the Receiver should set `rtp_enabled` in the second set of parameters to false and transport parameters in the SDP file should be mapped onto that first set of parameters in the array with `rtp_enabled` set to true (bearing in mind that `transport_params` in the same request take priority).
