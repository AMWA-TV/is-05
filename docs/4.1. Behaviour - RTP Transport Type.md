...
Where a Receiver supports SMPTE 2022-7 but is required to Receiver a non-SMPTE 2022-7 stream, only the first set of transport parameters should be used. `rtp_enable` in the second set of parameters must be set to false and transport parameters in the SDP file should be mapped onto that first set of parameters in the array.
